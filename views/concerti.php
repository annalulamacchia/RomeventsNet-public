...
<body class="text-center mx-auto">
  ...
  <div class="row mx-auto my-2">
    <form name="filter" method="POST" action="" class="d-inline-flex text-start" enctype="multipart/form-data">
      <input type="submit" name="RockInRoma" value="Rock In Roma"
        class="button1 col-lg-2 col-md-3 col-sm-3 col-xs-3 col-xxs-3"></input>
      <input type="submit" name="RomaSummerFest" value="Roma Summer Fest"
        class="button1 col-lg-2 col-md-3 col-sm-3 col-xs-3 col-xxs-3"></input>
      <input type="submit" name="Tutti" value="Elimina filtri"
        class="button1 col-lg-2 col-md-3 col-sm-3 col-xs-3 col-xxs-3"></input>
      <div class="col-lg-5 col-md-2 col-sm-1 col-xs-1 col-xxs-0"> </div>
      <ul class="nav-item p-0 m-0 col-lg-1 col-md-1 col-sm-1 col-xs-1 col-xxs-1 text-end">
        <ul class="navbar-nav p-1">
          <li class="nav-item dropdown">
            ...
            <ul class="dropdown-menu dropdown-menu-light" aria-labelledby="navbarDarkDropdownMenuLink">
              ...
              <li>
                <input name="ord_artista" type="submit" value="Artista" class="select1 dropdown-item ms-3"></input>
              </li>
              <li>
  ...
  if (isset($_POST['RockInRoma'])) {
    $sql = "SELECT* FROM Concerti WHERE RockInRoma='SI'";
    unset($_POST['RockInRoma']);
  } else if (isset($_POST['RomaSummerFest'])) {
    $sql = "SELECT* FROM Concerti WHERE RomeSummerFest='SI'";
    unset($_POST['RomaSummerFest']);
  } else if (isset($_POST['ord_artista'])) {
    $sql = "SELECT* FROM Concerti ORDER BY artista";
    unset($_POST['ord_artista']);
  } else if (isset($_POST['ord_data'])) {
    $sql = "SELECT* FROM Concerti ORDER BY substr (data,0,2) and substr(data,3,5) and substr(data,6,10) ASC";
    unset($_POST['ord_data']);
  } else if (isset($_POST['ord_luogo'])) {
    $sql = "SELECT* FROM Concerti ORDER BY luogo";
  ...
  $resultset = $concerti->query($sql);
  foreach ($resultset as $record) {
    $exp = explode(",", $record['immagine']);
    $exp2 = explode("+", $record['artista']);
    ?>
    <!--Carte con concerti-->
    <div class="card1 card1-xxs col-xxs-11">
      <a <?php echo "href='concerto.php?artista=" . $exp2[0] . "&data=" . $record['data'] . "'"; ?>>
        <img alt="" class="img1" src="<?php echo $exp[0]; ?>">
      </a>
      <div class="textBox">
        <div class="textContent">
          <a class="h1 h1-xxs" <?php echo "href='concerto.php?artista=" . $exp2[0] . "&data=" . $record['data'] . "'"; ?>><?php echo $record['artista']; ?></a>
        </div>
        <div class="row">
