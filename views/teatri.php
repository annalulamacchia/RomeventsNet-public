...
<body class="text-center">
  ...
  $teatri = new PDO('sqlite:../db/databaseREN');
  if (isset($_POST['ord_artista'])) {
    $sql = "SELECT* FROM Teatro ORDER BY artista";
    unset($_POST['ord_artista']);
  } else if (isset($_POST['ord_data'])) {
    $sql = "SELECT* FROM Teatro ORDER BY substr (data,0,2) and substr(data,3,5) and substr(data,6,10) ASC";
    unset($_POST['ord_data']);
  } else if (isset($_POST['ord_luogo'])) {
    $sql = "SELECT* FROM Teatro ORDER BY luogo";
